...
export default async function PembayaranPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  // 5. ✨ PROSES SANITASI DATA MATANG (Sudah bersih dari typo 'rawNasional' dan aman dari TS2304)
  const pesanan = {
    ...rawPesanan,
        ...
        ? {
          ...
          foto: dp.paket.foto
            ? typeof dp.paket.foto === "string"
              ? dp.paket.foto
              : `data:image/jpeg;base64,${Buffer.from(dp.paket.foto as Buffer).toString("base64")}`
            : null,
        }
        : null,
    })),
    jenisPembayaran: rawPesanan.jenisPembayaran
      ? {
        ...
        detailJenisPembayarans: (rawPesanan.jenisPembayaran as any).detailJenisPembayarans?.map((djp: any) => ({
          ...djp,
          logoPembayaran: djp.logoPembayaran
            ? typeof djp.logoPembayaran === "string"
              ? djp.logoPembayaran
              : `data:image/png;base64,${Buffer.from(djp.logoPembayaran as Buffer).toString("base64")}`
            : null,
        })) || [],
      }
      : null,
